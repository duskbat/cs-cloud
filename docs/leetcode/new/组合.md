...
class Solution {
    ...
    private void backtrace(StringBuilder sb, String digits) {
        int curIdx = sb.length();
        if (sb.length() == digits.length()) {
            res.add(sb.toString());
            return;
        }
        String s = map[digits.charAt(curIdx) - '0'];
...
class Solution {
    ...
    private void backtrace(int n, int m, StringBuilder sb, List<String> res) {
        if (n == 0 && m == 0) {
            res.add(sb.toString());
            return;
        }
        ...
        if (n > 0) {
            sb.append("(");
            backtrace(n - 1, m, sb, res);
            sb.deleteCharAt(sb.length() - 1);
        }
        // 试试 ")", 用过的"("更多, 剩下的")"更多
        if (m > n) {
            sb.append(")");
            backtrace(n, m - 1, sb, res);
            sb.deleteCharAt(sb.length() - 1);
        }
    }
}
...
class Solution {
    ...
    public List<List<Integer>> combinationSum(int[] arr, int target) {
        Arrays.sort(arr);
        backtrace(arr, target, new ArrayList<>(), 0);
        return res;
    }

    private void backtrace(int[] arr, int target, List<Integer> ele, int start) {
        if (target == 0) {
        ...
        for (int i = start; i < arr.length; i++) {
            int e = arr[i];
            if (target < e) return; // 剪枝
            target -= e;
            ele.add(e);
            backtrace(arr, target, ele, i);
            target += e;
...
class Solution {
    ...
    public List<List<Integer>> combinationSum2(int[] arr, int target) {
        ...
        backtrace(arr, new ArrayList<>(), 0, target);
        return res;
    }

    private void backtrace(int[] arr, List<Integer> ele,
        ...
        for (int i = start; i < arr.length; i++) {
            ...
            // 如果前一个没选, 那选后一个跟选前一个一样
            if (i > 0 && arr[i] == arr[i - 1] && !select[i - 1]) continue;

            select[i] = true;
            ele.add(arr[i]);
            backtrace(arr, ele, i + 1, target - arr[i]);
            select[i] = false;
            ele.remove(ele.size() - 1);
        }
    }
...
class Solution {
    ...
    private void backtracking(int start, int count, final int n) {
        if (count == 0) {
            combinations.add(new ArrayList<>(elements));
            return;
        }
        // count=1时就不需要给后面留位置了
        // count=1时, ?=n, 所以?=n-count+1
