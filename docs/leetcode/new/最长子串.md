...
> 这些情况下, 需要用朴素的方式向两端扩展求出臂展; 但是不需要写到if-else里

> 在插入 char 之后, 有了一些性质:
>
> -   `原回文串长度 = 臂展长度`: 例如 `len("#1#1#") = 2`, `len("#1#") = 1`
> -   `原左端点 = 新左端点/2 = (i-新臂展)/2`: 例如 新构造的左侧为 `"^#1#"`, `原左端点 = 新的左端点/2 = (i-新臂展)/2 = (2-1)/2 = 0`,
...
> 如果栈中会存入 '(', 那么随着 '(' 的 pop, 会不知道起始位置; 此时栈底部存一个 ')' 就能知道起始位置在哪了  
