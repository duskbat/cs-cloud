...
class Solution {
    public int reverse(int x) {
        ...
        while (x != 0) {
            ...
            // 负数:最后的余数范围是[-2,-1], 显然大于-8; 正数:最后的余数范围是[1,2], 显然小于7;
...
例如，输入12，1～12 这些整数中包含 1 的数字有1,10,11,12, 1一共出现了5次。
...
**解法2: DP**
> 三根指针, 分别指向需要与2,3,5相乘的位置, 每一步从3个乘积中选最小的, 乘完之后响应的指针++
